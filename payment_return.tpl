{if $status == 'OK'}
<p>{l s='Votre commande a été un succés sur %s' sprintf=$shop_name mod='offlinecardpayment'}
		<br /><br />
		
		<br /><br />- {l s='Montant' mod='offlinecardpayment'} <span class="price"> <strong>{$total_to_pay}</strong></span>
...
{else}
	<p class="warning">
		{l s="Une erreur au niveau de paiement a été detectée et la commande n'a pas passé. Veuillez contacter le " mod='offlinecardpayment'} 
		<a href="{$link->getPageLink('contact', true)|escape:'html'}">{l s='support.' mod='offlinecardpayment'}</a>.
	</p>
{/if}